...
const unsigned idMask = 0x0001ffff;
const unsigned addOne = idMask + 1;
const unsigned countMask = 0xfffe0000; // TODO ~idMask?

bool compareCompactHashMatch2(unsigned int p1, unsigned int p2) {
	return ((p1 & countMask) > (p2 & countMask));
}


template<typename Dtype>
WTACacheAware<Dtype>::WTACacheAware(int nSectionsPerHash_,
...
void invertHashes(int M_, int nHashes, vector<int> * allCurHashesInverted,
...
void invertMatrixArrayOfVectorForm(int N_, int nHashes,
...
template<typename Dtype>
void WTACacheAware<Dtype>::makeNewHashTable(const FakeSpan<Dtype> & data) {
	...
	invertMatrixArrayOfVectorForm(n, nHashes,
			allCurHashesInverted, allCurHashes);
...
template<typename Dtype>
void WTACacheAware<Dtype>::getWTAHashFaster(std::vector<int> &hashes,
		const Dtype* data) {
	int curPermPoint = 0;
...
template<typename Dtype>
void WTACacheAware<Dtype>::putInvertedHashesIntoManualTable(
	...
	for (int j = 0; j < nHashes; j++) {
		...
		for (unsigned int i = 0; i < N_; i++) {
			manualHashTable[j * manHashWidth + allCurHashesInverted[j][i]]++;
		}
		//	  if (j != 0){
...
template<typename Dtype>
void WTACacheAware<Dtype>::lookUpHashForTheseVectors(
...
template<typename Dtype>
void WTACacheAware<Dtype>::lookupHashTableForTheseHashes(
	...
	invertHashes(M_, nHashes, (vector<int> *) allCurHashesInverted,
			(vector<int> *) allCurHashes);
...

	{
		...
		for (int i = 0; i < M_; i++) {
			...
			std::nth_element(myHashMap, myHashMap + numberToAccept,
					myHashMap + mapSize, compareCompactHashMatch2);
			int j = 0;
...
template<typename Dtype>
void WTACacheAware<Dtype>::calculateNewHashPermutations() {
...
template class WTACacheAware<float>;
