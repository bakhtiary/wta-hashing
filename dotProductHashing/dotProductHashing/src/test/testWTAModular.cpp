...
TEST(WTAModularTest, testTopPicks){
	...
	std::unique_ptr<Hasher<float>> hasher( new HasherSimple<float>(nSectionsPerHash, sectionSize, nHashes, sizeOfEachVector,1));
	std::unique_ptr<HashStore> store(new HashStoreSimple());
	std::unique_ptr<Counter> counter(new CounterSimple());


	WTAModular<float> tester(
			hasher,store, counter , nAccept);
	...
	FakeSpan <float>  data2Span (data.data(),M,K);
